...
async def save_time_handler(message: Message, state: FSMContext) -> None:
    ...
    time_send = message.text.strip()
    if not time_send:
        await message.answer("Строка не может быть пустая. Попробуйте снова.")
        return

    await state.update_data(prompt=time_send)
    data["time_send"] = time_send
    ...
    try:
        ...
        with db.atomic():
            TimeSend.insert(user_id=message.from_user.id, time_send=time_send).on_conflict(
                conflict_target=[TimeSend.user_id], update={TimeSend.time_send: time_send}).execute()
